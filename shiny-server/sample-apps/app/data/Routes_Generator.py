...
def decode_polyline(polyline_str):
    """Pass a Google Maps encoded polyline string;
...
def generate_geojson(breweryData, id_1, id_2, mode):
    directions_result = \
        gmaps.directions(
            [breweryData[breweryData.brewery_id == id_1]['latitude'],
             breweryData[breweryData.brewery_id == id_1]['longitude']],
            [breweryData[breweryData.brewery_id == id_2]['latitude'],
             breweryData[breweryData.brewery_id == id_2]['longitude']],
            mode=mode,
    ...
    if mode == 'driving':
        ...
        coord['geometry']['coordinates'] = \
            [[item[1], item[0]] for item in decode_polyline(
...
gmaps = googlemaps.Client(key='GGMAP_KEYS')  # change the key
...
#     finaloutput[str(i[0]) + '_' + str(i[1]) + '_' + 'transit'] = generate_geojson(breweryData, i[0], i[1], 'transit')
    finaloutput[str(i[0]) + '_' + str(i[1]) + '_' + 'driving'] = \
        generate_geojson(breweryData, i[0], i[1], 'driving')
...
#     finaloutput[str(i[1]) + '_' + str(i[0]) + '_' + 'transit'] = generate_geojson(breweryData, i[1], i[0], 'transit')
    finaloutput[str(i[1]) + '_' + str(i[0]) + '_' + 'driving'] = \
        generate_geojson(breweryData, i[1], i[0], 'driving')
